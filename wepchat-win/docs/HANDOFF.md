...
> 最后更新：2026-07-23（SQLite S1/S2、聊天渲染 P0/P1、隔离 HTML 预览与本轮实测修复）  
...
下一阶段首选工作是 `docs/sqlite-storage-plan.md` 的 S3：历史消息分页 + FTS5 搜索；也可继续 `docs/deepsearch.md` 的 P2 截图能力。
...
| 文档 | 状态 |
| --- | --- |
| `docs/sqlite-storage-plan.md` | 存储改造实施依据；文末 S1/S2 记录是当前状态，S3 待做 |
...
1. `session_messages_page(sessionId, beforeSeq, limit)`。
2. 前端 prepend 旧消息并保持滚动锚点。
3. FTS5 全局消息搜索。
4. 退出时 `wal_checkpoint(TRUNCATE)`。
5. 做计划中的 100×500、长会话写放大和分页性能基准；当前已完成正确性测试，但尚未正式跑完整性能验收。
...
```text
M  docs/deepsearch.md
M  src-tauri/Cargo.lock
M  src-tauri/Cargo.toml
M  src-tauri/src/lib.rs
M  src-tauri/src/preview_server.rs
M  src-tauri/src/sessions.rs
M  src-tauri/tauri.conf.json
M  ui/css/app.css
M  ui/index.html
M  ui/js/app.js
M  ui/js/browser-preview.js
M  ui/js/markdown.js
?? docs/sqlite-storage-plan.md
?? src-tauri/src/db.rs
?? ui/dev/
?? ui/js/chat-rail.js
?? ui/js/chat-scroll.js
?? ui/js/chat-view.js
?? ui/libs/katex/
?? image.png
```
...
```text
继续 wepchat-win。先完整阅读 docs/HANDOFF.md、docs/sqlite-storage-plan.md；需要处理聊天长期性能时再读 docs/deepsearch.md。

当前：SQLite S1/S2 已落地并通过实测；稳定聊天 DOM/流式滚动/rail/KaTeX/task list 已完成；手动 HTML 与代码块 artifact 预览均已实测可用。工作树未提交，勿覆盖用户改动。

优先继续 SQLite S3：session_messages_page + prepend 保位 + FTS5；任何 schema 变化必须走 user_version 迁移。预览 iframe 当前必须保留 allow-scripts allow-same-origin，原因与安全边界见 HANDOFF。
```
