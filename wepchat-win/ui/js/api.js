...
const API = (() => {
  ...
  /* ---------- 传输层：Tauri/Rust 优先（规避 WebView CORS），否则 XHR ---------- */
  ...
  function feedSse(bufState, chunk, onEvent) {
    bufState.buf += chunk;
    let idx;
    while ((idx = bufState.buf.indexOf('\n')) >= 0) {
      let line = bufState.buf.slice(0, idx);
      bufState.buf = bufState.buf.slice(idx + 1);
      if (line.endsWith('\r')) line = line.slice(0, -1);
      if (line === '') { bufState.curEvent = ''; continue; }
      if (line.startsWith('event:')) { bufState.curEvent = line.slice(6).trim(); continue; }
      if (line.startsWith('data:')) {
        const data = line.slice(5).trim();
        if (data === '[DONE]') continue;
        try {
          onEvent(bufState.curEvent, JSON.parse(data));
          bufState.parsedSse = true;
        } catch (e) { /* 非 JSON 行忽略 */ }
      }
    }
  }

  function tryJsonFallback(raw, onEvent) {
  ...
  function httpStatusCode(status) {
    ...
    if (n === 408) return 'HTTP-408';
    if (n === 429) return 'HTTP-429';
    if (n >= 500) return 'HTTP-5XX';
    return 'HTTP-' + n;
  }

  /* ---------- SSE 流式请求（Rust 代理 / XHR 增量解析，abort 可中断） ---------- */
  function sseOnceViaTauri({ url, headers, body, onEvent, signal }) {
    return new Promise(async (resolve, reject) => {
      const requestId = newRequestId();
      const bufState = { buf: '', curEvent: '', parsedSse: false };
      let seen = 0, rawAll = '', aborted = false, settled = false, forcedError = null;
      let firstByteTimer = null, idleTimer = null, unlisten = null;
      ...
      const failAfter = (code, message) => {
        ...
        aborted = true;
        tauriInvoke('http_stream_abort', { args: { requestId } }).catch(() => {});
        settle(() => reject(forcedError));
      };
      const armIdle = () => {
        clearTimeout(idleTimer);
      ...
      const finishOk = () => {
        if (!bufState.parsedSse) tryJsonFallback(rawAll || bufState.buf, onEvent);
        settle(() => resolve());
      };
      ...
      if (signal) {
        if (signal.aborted) { settle(() => resolve()); return; }
        signal.addEventListener('abort', onAbort, { once: true });
      }

      try {
        unlisten = await tauriListen('http-stream', (event) => {
          const p = (event && event.payload) || {};
          if (p.requestId !== requestId) return;
          if (settled) return;

          if (p.event === 'start') {
            /* status known; wait for chunks */
            return;
          }
          if (p.event === 'chunk') {
            const chunk = p.chunk || '';
            ...
            rawAll += chunk;
            seen += chunk.length;
            feedSse(bufState, chunk, onEvent);
            armIdle();
            return;
          }
          if (p.event === 'done') {
          ...
          if (p.event === 'error') {
            if (aborted && !forcedError) {
              settle(() => resolve());
              return;
            }
            const status = Number(p.status) || 0;
            const bodyText = p.chunk || '';
            if (status >= 400) {
              settle(() => reject(NetStability.createError(
                httpStatusCode(status),
                extractError(bodyText, status),
                { status, url, receivedBytes: seen }
              )));
              return;
            }
            const msg = p.message || '网络请求失败，请检查网络与接口地址';
            if (/取消|abort|cancel/i.test(msg) && !forcedError) {
              settle(() => resolve());
              return;
            }
            settle(() => reject(NetStability.createError(
              /超时|timeout/i.test(msg) ? 'NET-TIMEOUT' : 'NET-CONNECT',
              msg,
              { url, receivedBytes: seen }
            )));
          }
        });
      } catch (err) {
        settle(() => reject(NetStability.createError('NET-CONNECT', err.message || '无法监听流式事件', { url })));
        return;
      }
      ...
      try {
        // Tauri 按命令参数名解包：http_stream(args: HttpStreamArgs) → { args: {...} }
        await tauriInvoke('http_stream', {
          args: {
            requestId,
            method: 'POST',
            url,
        ...
        /* command returns after stream ends; event handler usually already settled */
        if (!settled) finishOk();
      } catch (err) {
        if (settled) return;
        if (aborted && !forcedError) {
        ...
        settle(() => reject(NetStability.createError(
          'NET-CONNECT',
          (err && err.message) || String(err || '网络请求失败'),
          { url, receivedBytes: seen }
        )));
      }
    });
  ...
  function sseOnceViaXhr({ url, headers, body, onEvent, signal }) {
    return new Promise((resolve, reject) => {
      ...
      let seen = 0, aborted = false, forcedError = null;
      const bufState = { buf: '', curEvent: '', parsedSse: false };
      ...
      xhr.onprogress = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          if (t.length > seen) {
            clearTimeout(firstByteTimer);
            feedSse(bufState, t.slice(seen), onEvent);
            seen = t.length;
            armIdle();
          }
        }
      ...
      xhr.onload = () => {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          const t = xhr.responseText;
          if (t.length > seen) feedSse(bufState, t.slice(seen), onEvent);
          /* 服务端可能忽略 stream 参数直接返回 JSON */
          if (!bufState.parsedSse) tryJsonFallback(t, onEvent);
          resolve();
        } else {
  ...
  function sseOnce(options) {
    if (hasTauriHttp()) return sseOnceViaTauri(options);
    return sseOnceViaXhr(options);
  }
  ...
  async function plainRequestViaTauri(method, url, headers, body) {
    const hdrs = Object.assign({}, headers || {});
    if (body) hdrs['Content-Type'] = hdrs['Content-Type'] || 'application/json';
    // Tauri 按命令参数名解包：http_request(args: HttpRequestArgs) → { args: {...} }
    ...
    throw new Error(extractError(res.body, res.status));
  }

  function plainRequestViaXhr(method, url, headers, body) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
  ...
  function plainRequest(method, url, headers, body) {
    if (hasTauriHttp()) return plainRequestViaTauri(method, url, headers, body);
    return plainRequestViaXhr(method, url, headers, body);
  }
