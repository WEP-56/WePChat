...
WepChat 目前仍是静态 Vue/H5 主体，按 HBuilderX/uni-app App 打包方向推进，只做安卓。入口是 `index.html`，核心风格保持“克制、轻量、快速”：偏日常随手生成、快速验证想法，不做 ComfyUI、Midjourney 或图库类重型工作站。
...
针对 Android 实机反馈的两个问题：

1. 生图存入工作区提示“存储空间不足”：根因是工作区/会话整体存在 localStorage，Android WebView 配额仅约 5MB，base64 图片很快撑爆（与设备磁盘空间无关）。`js/store.js` 已改为 IndexedDB + 内存缓存（同步 API 不变），启动时自动把旧 localStorage 数据迁入并释放原配额；`js/app.js` 挂载前 `await Store.init()`。
2. 导出（相册/分享/下载目录）点击后无任何反应：根因是 App 端三条导出链路都汇聚到 HTML5+ `FileWriter.write(blob)`，而 FileWriter 只支持写字符串，传 Blob 时回调不触发，Promise 永久挂起（连失败 toast 都出不来）。第一版尝试 Native.js（`Base64.decode` + `FileOutputStream`）实机报 `NullPointerException`——Native.js 的 byte[] 跨桥会变成 null，此路不通。`js/util.js` 最终方案改为全程不让二进制过 JS 桥：
   - 图片：`plus.nativeObj.Bitmap.loadBase64Data` + `bitmap.save(path)` 原生解码落盘（jpg 按扩展名保存，其余统一 png 编码）。
   - 文本：`FileWriter.write(字符串)`（官方支持的唯一可靠写法）。
   - ZIP：文件先暂存 `_doc/` 临时目录，再 `plus.zip.compress` 原生压缩输出到下载目录（H5 端仍用 JS 生成 ZIP Blob）。
   - `fetch(dataUrl)` 全部换成同步 `U.dataUrlToBlob`，规避 WebView 对 data: URL fetch 的兼容问题。
   - 公共下载目录写入与存相册前调用 `plus.android.requestPermissions` 申请存储权限（Android ≤10 需要；11+ 拒绝也不影响公共目录写入）。
   - 所有 plus 桥接操作加超时兜底，失败一定会弹“导出失败”toast，不再静默。
   - 分享图片的临时文件改放 `PRIVATE_DOC`（不再借道公共下载目录）。
   - `manifest.json` 的 Android permissions 增加 `WRITE/READ_EXTERNAL_STORAGE`。

以上需要重新云打包/自定义基座后实机回归（Invocation/Native.js 权限已在 manifest 中）。
